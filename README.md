...
- Before publication, the refresh compares product totals, all six root-category totals, per-retailer product coverage, and the total active retailer-offer count with the last live catalogue. The default limits reject a catalogue-wide drop above 3%, a root-category drop above 5% (at least 10 products), a retailer-coverage drop above 20% (at least 50 products from a baseline of 100), or an offer-count drop above 8% (at least 100 offers). A nonzero retailer-only change can establish a new baseline after six consistent complete snapshots spanning at least six hours. Root-category, total-offer, vanished-retailer, and whole-catalogue anomalies remain independently blocked. For a supervised one-run retailer repair, `POSOKANEI_APPROVE_RETAILER_BASELINE=retailer_id:count npm run live:refresh` accepts only exact counts for every anomalous retailer and still runs the whole-catalogue contraction guard. The legacy `POSOKANEI_ALLOW_CATALOG_CONTRACTION=1` emergency override bypasses all publication guards and should not be used for ordinary baseline changes. Diagnostics and confirmation progress are written to `refresh-status.json`.
